...
def create_model_graph(model_info):
  ...
  with tf.Graph().as_default() as graph:
    file_name = FLAGS.model_file_name or model_info['model_file_name']
...
def create_model_info(architecture, model_download_url):
 ...
 but found '%s' for architecture '%s'""",
    ...
    if len(parts) == 3:
      ...
      is_quantized = True
    data_url = (
        f'{model_download_url}mobilenet_v1_{version_string}_{size_string}' +
        '_frozen.tgz')
    print(data_url)
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # constants.
    save_graph_to_file(sess, graph, os.path.join(FLAGS.output_dir,FLAGS.output_graph))
    with gfile.FastGFile(os.path.join(FLAGS.output_dir,FLAGS.output_labels), 'w') as f:
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--model_download_url',
      type=str,
      default='https://raw.githubusercontent.com/rakelkar/models/master/model_output/',
      help="""\
  ...
  parser.add_argument(
      ...
      default='modeloutput',
      help="""\
      Directory to drop the (re) trained model into.\
      """
  )
  FLAGS, unparsed = parser.parse_known_args()
  print("image directory:{}".format(FLAGS.image_dir))
